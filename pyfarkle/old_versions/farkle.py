...
class Player():
    def __init__(self, ai_flag=False):
        self.total_score = 0
...
class GameEngine():
    def __init__(self, player_index, total_score, ai_flag=False):
        ...
        while True:
            ...
            if self.initial_farkle_check():
                ...
                if self.ai_flag:
                    ...
                    choice = input(">>>: ").lower()
                if choice or self.ai_flag:
                    if not self.ai_flag:
                        choice = choice.split()
                        choice = self.choose_all(choice)
                    if self.exact_match(choice) or self.ai_flag:
                        if not self.ai_flag:
                        ...
                        if not self.idiot_proof():
                            while True:
                                ...
                                if not self.ai_flag:
                                    ...
                                    #Turn ai choice into proper dice display..maybe
                                    print("The computer has chosen {} dice worth {}pts.".format(len(self.dice), self.round_points))
                                    confirm = 'y'
                                    time.sleep(7)
    ...
    def continued_roll(self):
        ...
        entropy = random.randint(2, 3)
        if self.ai_flag:
            if amount_to_roll > entropy or self.enter_game == False:
    ...
    def ai_decision(self):
        ai_choice = self.ai_calc_final_choice()
        return ai_choice
            
            
    def ai_calc_final_choice(self):
        ...
        while True:
            ...
            final_key = max_point_key[0]
            if not self.idiot_proof():
                final_dice = combinations[final_key]
                self.round_points = 0
                return final_dice
            else:
                del score_combo_dict[final_key]
        
        #clean up code, have ai keep rolling until first 500 points are scored to get into game before rules kick in
    def ai_calc_poss_choices(self):
        dice = self.dice_initial
        dice_2dtuplists = [list(itertools.combinations(dice, i+1)) for i in range(len(dice))]
        unified_tup_list = []
...
def run_game(players, amount_player):
    ...
    while True:
        ...
        for player_index in range(amount_player):
            current_player = GameEngine(player_index, players[player_index].total_score, players[player_index].ai_flag)
...
def singleplayer_initialize():
    while True:
        try:
            amount_player = int(input("\n\n\t\tHow many computer players would you like to play?: "))
            clear_screen()
            if 1 <= amount_player < 86:
                players = [Player(True) for i in range(amount_player)]
                players.insert(0, Player())
                return players, amount_player+1
            else:
                print("\n\n\t\tPlease choose a number between 1 and 85.")
        except:
            clear_screen()
            print("\n\t\tPlease enter a whole number only.")
...
if __name__=="__main__":
    while True:
        ...
        if menu_option == '1':
            clear_screen()
            players, amount_player = singleplayer_initialize()
            run_game(players, amount_player)
